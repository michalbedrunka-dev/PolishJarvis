...
def _resolve_browser_profile(browser: str) -> tuple[str, Optional[str]]:
    """Return the Chromium user-data dir and preferred profile name for a browser."""
    profile_root = _real_profile_dir(browser)
    root_path = Path(profile_root)

    if _OS == "Windows":
        if root_path.exists():
            for profile_name in ("Default", "Profile 1", "Profile 2"):
                if (root_path / profile_name).exists():
                    return str(root_path), profile_name
            if (root_path / "Default").exists() or any(root_path.iterdir()):
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        profile_root, profile_dir = _resolve_browser_profile(self.browser_name)
        ...
        if profile_dir:
            kwargs["args"].append(f"--profile-directory={profile_dir}")
        ...
        try:
            self._context = await engine_obj.launch_persistent_context(profile_root, **kwargs)
            await asyncio.sleep(0.5) 
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] profile={profile_root} ({profile_dir or 'default'})")
            return
        except Exception as e:
